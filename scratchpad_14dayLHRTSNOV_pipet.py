...
e_shockR = [
 '/home/vburns/Dropbox/ConchisData/2013-02-17/f00160/f00160_2013-02-17-10-53-08.json',
...
e_shockN = [
 '/home/vburns/Dropbox/ConchisData/2013-02-17/f00168/f00168_2013-02-17-14-20-02.json',
...
e_RTN= [
 '/home/vburns/Dropbox/ConchisData/2013-02-17/f00168/f00168_2013-02-17-15-09-57.json',
...
e_RTN = aba.loadMultipleDataFiles(e_RTN)

e_long = [
 '/home/vburns/Dropbox/ConchisData/2013-02-17/f00168/f00168_2013-02-17-15-50-10.json',
...
(e_fracRT,e_distRT) = aba.getSidePreference_Multi(e_RTN)
...
#convert to array
eBV = np.array([np.array([eBaseVelN[n]]) for n in range(len(eBaseVelN))])
eSSV = np.array([np.array([eStartShockVelN[n]]) for n in range(len(eStartShockVelN))])
eSV = np.array([np.array([eShockVelN[n]]) for n in range(len(eShockVelN))])
eLH = np.array([np.array([eLHVelN[n]]) for n in range(len(eLHVelN))])
...
pylab.plot(0, [eBaseVelN], 'r.')
pylab.plot(1, [eStartShockVelN], 'r.')
pylab.plot(2,[eShockVelN], 'r.')
pylab.plot(3,[eLHVelN], 'r.')
pylab.plot(4,[eVellate],'r.')
pylab.plot([0,1,2,3,4],[np.mean(eBaseVelN), np.mean(eStartShockVelN), np.mean(eShockVelN),scipy.stats.nanmean(eLHVelN), np.mean(eVellate)],'o-k', lw=3)
pylab.axvline(2.5, ls= '-', c ='k', lw=2)
