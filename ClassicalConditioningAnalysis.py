...
def loadDataFromFile(filename, arena_mm = [(0,0),(0,35),(80,35),(80,0)]):
	f = open(filename)
...
def loadDataFromFile_AvgMultiFish(filename, arena_mm = [(0,0),(0,35),(80,35),(80,0)]):
	f = open(filename)
	jsonData = json.load(f)
	f.close()
...
def loadData(fishDir, arena_mm = [(0,0),(0,35),(80,35),(80,0)]):
	filelist = os.listdir(fishDir)
	filelist = filter(lambda x: x.endswith('json'), filelist)
	if len(filelist) != 1:
		print 'Multile json files found'
		return None 
	return loadDataFromFile(fishDir + os.sep + filelist[0], arena_mm)
...
def plotFishPosition_old(jsonData):
	state = jsonData['stateinfo']
	st = state[0][0]
	et = state[-1][0]
	fishTracking = getTracking(jsonData)
	frametime = fishTracking[:,0] - st
...
def getSidePreference(runData, dividePoint=40, cond=3, color='Red'):
	...
	for switchNdx in range(nS,nE+1):
		...
		if color.lower() == state[switchNdx][2].lower():
			timeOnColor1.append(timeOnSide1[-1])
		elif color.lower() == state[switchNdx][3].lower():
